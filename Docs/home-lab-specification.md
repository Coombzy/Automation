...
| **Workloads** | Cron/heartbeats, docs, delegation, light inference, Mission Control docker stack (initial), Nextcloud, n8n, Matrix (Phase 0/1) |
...
- Compose stack: Postgres, Nextcloud, Matrix (placeholder), n8n, MC app  
...
### 7.2 Docker / services
- Secrets via `.env` outside git (`POSTGRES_PASSWORD`, `NEXTCLOUD_ADMIN_PASSWORD`, `N8N_PASSWORD`)  
