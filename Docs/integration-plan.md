...
|------|---------|----------------|
| Host / edge services | **Porsche (M4 Pro)** | Primary user-facing services while traveling (Nextcloud, Mission Control UI, n8n, Matrix initially) |
...
- **n8n** (planned): workflow glue between Nextcloud, Matrix, agents, alerts  
...
| **Orchestration** (n8n + agents + custom dashboard) | Automation rules, sync jobs, agent triggers, health checks |
...
- **Trigger:** n8n webhooks / cron heartbeats  
...
[Ben phone / Code Mater]
   ...
   Discord / HTTPS / VPN
        |
   [Porsche - edge host]
     Nextcloud, MC UI, n8n, Matrix (initial)
        |
   private network / VPN
   |                |
...
All external systems sit behind an **integration layer** (API adapters + n8n), never direct DB writes from third parties.
...
- [ ] n8n placeholder workflows for notify/sync  
